...
class LogisticRegression:
    def __init__(self, num_iter=100, learning_rate=0.3, reg_param=0.0,
        ...
        self.weight = ntorch.randn(VECTORS.shape[1], requires_grad=True,
                                   names=('vocab',))
        ...
        # training
        opt = torch.optim.Adam([self.weight.values, self.bias], lr=learning_rate)

        for i in range(num_iter):
            try:
                opt.zero_grad()
                y = self.weight.dot('vocab', x_train) + self.bias
    ...
    def __call__(self, text):
        ...
        for i, sent in enumerate(sentences):
            ...
            prob = (self.weight.dot('vocab', named_x).values + self.bias).sigmoid()
            results[i] = prob
    ...
    def transform_x(self, sent):
        embeds = VECTORS[sent]
        return ntorch.tensor(embeds.sum(dim=0), names=('vocab',))
